...
func (pm *PresenceManager) serializeRecord(record *PresenceRecord) ([]byte, error) {
	...
	return []byte(fmt.Sprintf("presence:%s:%s:%v", record.Bee, record.Handle, record.Addrs)), nil
}
...
func (pm *PresenceManager) serializeHandleIndex(record *PresenceRecord) ([]byte, error) {
	// In a full implementation, this would create a proper HandleIndex record
	// For now, we'll use a simple approach
	return []byte(fmt.Sprintf("handle:%s:%s", record.Handle, record.Bee)), nil
}
